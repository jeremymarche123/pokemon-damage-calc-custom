...
try {
    for (var _e = __values(Object.entries(SV)), _f = _e.next(); !_f.done; _f = _e.next()) {
        ...
        if (specie.otherFormes) {
            specie.otherFormes = __spreadArray([], __read(new Set(specie.otherFormes)), false).filter(function (f) { return !f.endsWith('-Gmax'); });
            if (!specie.otherFormes.length)
                specie.otherFormes = undefined;
        }
    }
}
